...
void UR1AssetManager::LoadPreloadAssets()
{
	...
	UR1PrimaryDataAsset* AssetData = nullptr;
	FPrimaryAssetType PrimaryAssetType(UR1PrimaryDataAsset::StaticClass()->GetFName());
	TSharedPtr<FStreamableHandle> Handle = LoadPrimaryAssetsWithType(PrimaryAssetType);
	if (Handle.IsValid())
	{
		Handle->WaitUntilComplete(0.f, false);
		AssetData = Cast<UR1PrimaryDataAsset>(Handle->GetLoadedAsset());
	}
...
{
	UR1AssetManager& AssetManager = Get();
	UR1PrimaryDataAsset* LoadedAssetData = AssetManager.LoadedAssetData;
